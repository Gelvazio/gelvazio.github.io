...
O arquivo `llms.txt` contém a documentação técnica completa do projeto — é o espelho externo da seção "Como este projeto funciona" deste `AGENTS.md`. Sempre que a seção abaixo for atualizada, o `llms.txt` deve ser atualizado com o mesmo conteúdo (em formato texto puro, sem markdown de títulos).
...
├── AGENTS.md           ← Instruções para o Codex (este arquivo)
├── js/
